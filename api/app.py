...
if page == "🔍 Detection":
    ...
    if st.session_state.df_pred is not None:
        ...
        col4.markdown(f"<div class='card'><div>Risk Level</div><div class='kpi-number'>{risk}%</div></div>", unsafe_allow_html=True)

        st.write("")

        if attacks > 0:
            st.markdown("<div class='alert-danger'>🚨 ACTIVE THREAT DETECTED</div>", unsafe_allow_html=True)
        else:
